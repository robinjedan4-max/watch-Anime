...
export default function AnimeLibrary({ onProfile, onExplore, onLogout, currentUser }) {
  ...
  useEffect(() => {
    const savedLikes = localStorage.getItem(`likes_${currentUser}`);
    const savedDislikes = localStorage.getItem(`dislikes_${currentUser}`);
    const savedComments = localStorage.getItem(`comments_${currentUser}`);
  ...
  useEffect(() => {
    localStorage.setItem(`likes_${currentUser}`, JSON.stringify(likes));
  }, [likes, currentUser]);

  useEffect(() => {
    localStorage.setItem(`dislikes_${currentUser}`, JSON.stringify(dislikes));
  }, [dislikes, currentUser]);
                    ...
                    {(comments[show.id] || []).map((comment) => (
                      <div key={comment.id} className="comment-item">
                        ...
                        <small className="comment-meta">
                          <span className="comment-user">{comment.user}</span> • {comment.time}
                        </small>
                      </div>
                    ))}
                  </div>
